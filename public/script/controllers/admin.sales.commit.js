module.exports = function(app){
	app.controller('AdmSalesCommitController', ['$scope', '$http', 'API_URL', 'BASE_URL', '$window',
		function($scope, $http, API_URL, BASE_URL, $window){

			$scope.initData = function($datas){
				$scope.salesheader = JSON.parse($datas);
				$scope.salesheader.created_at = $scope.makeDateTime($scope.salesheader.created_at);
				$.each($scope.salesheader.salesdetail, function($i, $ii){
					$salesdetail.created_at = $scope.makeDateTime($salesdetail.created_at);
					$salesdetail.cartheader.created_at = $scope.makeDateTime($scope.salesdetail.cartheader.created_at);
					$.each($salesdetail.cartheader.cartfile, function($index, $item){
					...
					$.each($salesdetail.cartheader.cartpreview, function($index, $item){
						$item.waiting = false;
						$item.created_at = $scope.makeDateTime($item.created_at);
						$.each($item.file, function($index2, $item2){
							$item2.created_at = $scope.makeDateTime($item2.created_at);

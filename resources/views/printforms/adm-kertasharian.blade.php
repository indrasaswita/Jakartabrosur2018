...
<body class="helvetica">
	...
	<div style="height: 1097px;margin: 1px;">
		<table class="width-100">
			<tr>
				<td class="size-24 italic width-80">
					<div class="width-80"><b>Daftar Kertas Harian</b> - Jakartabrosur.com</div>
				</td>
		...
		<table class="width-100 text-left size-14 v-top">
			<tr>
				<th colspan="2"  style="border-top: 1px solid #999;">
					{{$detail['jobtitle']}}
				</th>
				<td style="border-top: 1px solid #999; text-align: right !important;">
					uk. Plano
				</td>
			...
			<tr>
				<td>
					{{number_format($detail['quantity'], 0, '.', ',')}} {{$detail['quantitytypename']}} jadi
				</td>
				<td>
					<b>{{$detail['papername']}} {{$detail['gramature']}}gsm</b> ({{$detail['color']}})
				</td>
				<td class="text-xs-right">
					<b>{{number_format($detail['planowidth'], 0, '.', ',')}} x {{number_format($detail['planolength'], 0, '.', ',')}}</b> cm
				</td>
				<td>
					-> <b>{{number_format($detail['printwidth'], 1, '.', ',')}} x {{number_format($detail['printlength'], 1, '.', ',')}}</b> cm
				</td>
			</tr>
			<tr>
				<td>
					Total <b>{{number_format($detail['totalplano'], 0, '.', ',')}} lembar</b> plano
				</td>
				<td>
					&plusmn; Rp {{number_format($detail['totalpaperprice'], 0, '.', ',')}},-
				</td>
				<td colspan="2">
					{{$detail['totalinplanox']}} x {{$detail['totalinplanoy']}} + {{$detail['totalinplanorest']}} = <u>{{$detail['totalinplano']}} lembar</u> dalam 1 plano
				</td>
			</tr>
			<tr>
				<td colspan="2">
